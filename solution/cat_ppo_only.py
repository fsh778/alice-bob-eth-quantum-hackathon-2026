    ...
    def make_state(x_nom, Tz, Tx, Tz_prev, Tx_prev):
        ...
        return jnp.concatenate(
            [
                ...
                ),  # positive → Tx improved this step
            ]
        )
...
def main():
    ...
    parser.add_argument(
        "--opt-freq", type=int, default=3, help="PPO optimizer steps per drift step"
    )
    parser.add_argument("--horizon", type=int, default=64)
    parser.add_argument("--ppo-epochs", type=int, default=4)
    parser.add_argument("--lr-ppo", type=float, default=3e-3)
    ...
    print(
        f"   x = {x_static[:3]}  Tz={float(Tz_s0):.1f} µs  Tx={float(Tx_s0):.4f} µs\n"
    )
    ...
    for ax, col, amp, label, title in [
        ...
        ax.plot(
            st, slow[:, col], "r--", lw=1.4, alpha=0.8, label=f"Sinusoidal (±{amp} MHz)"
        )
        ax.fill_between(
            st, drift[:, col], slow[:, col], alpha=0.18, label="OU component"
        )
    ...
    ax = axes[1, 0]
    ax.plot(
        st, _sm(x_eff_ppo[:, 0] - ideal_g2, W), color="C4", label="PPO tracking error"
    )
    ax.plot(
        st,
        _sm(x_eff_static[:, 0] - ideal_g2, W),
        "k:",
        lw=0.8,
        alpha=0.5,
    ...
    ax = axes[1, 1]
    ax.plot(
        st, _sm(x_eff_ppo[:, 2] - ideal_eps, W), color="C4", label="PPO tracking error"
    )
    ax.plot(
        st,
        _sm(x_eff_static[:, 2] - ideal_eps, W),
        "k:",
        lw=0.8,
        alpha=0.5,
        label="Static error (= drift)",
