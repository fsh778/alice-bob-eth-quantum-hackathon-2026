...
def generate_drift_trajectory(n_steps: int, seed: int = 0,
                              period: int = DRIFT_PERIOD) -> np.ndarray:
    """
...
def run_static_cmaes(batched_loss, loss_fn, x0, n_epochs, batch_size):
    ...
    for ep in range(n_epochs):
        ...
        if ep % 10 == 0:
            print(f"    epoch {ep:3d}  loss={loss_h[-1]:.3f}  "
                  f"Tz={Tz_h[-1]:.1f}  Tx={Tx_h[-1]:.4f}")

    # Evaluate final mean
    _, Tx_f, Tz_f = loss_fn(jnp.array(opt.mean))
    print(f"    Final: g2={opt.mean[0]:.4f}, eps_d={opt.mean[2]:.4f}  "
...
def evaluate_under_drift(batched_loss, x_fixed: np.ndarray,
                         drift_traj: np.ndarray) -> tuple:
    """
...
def run_online_cmaes(batched_loss, loss_fn, x0: np.ndarray,
                     drift_traj: np.ndarray, batch_size: int,
                     sigma: float = 0.08, optimizer_freq: int = 1) -> tuple:
    """
...
def run_online_adam(loss_fn, x0: np.ndarray, drift_traj: np.ndarray,
                    lr: float, optimizer_freq: int = 1,
                    batched_loss=None) -> tuple:
    """
    ...
    grad_fn = jax.jit(
        jax.value_and_grad(loss_with_drift, argnums=0, has_aux=True)
    )
...
def run_online_adam_predictive(loss_fn, x0: np.ndarray, drift_traj: np.ndarray,
                                lr: float, optimizer_freq: int = 1,
                                lookahead: float = 0.5,
                                batched_loss=None) -> tuple:
    """
...
def run_online_cmaes_predictive(batched_loss, loss_fn, x0: np.ndarray,
                                 drift_traj: np.ndarray, batch_size: int,
                                 sigma: float = 0.08, optimizer_freq: int = 1,
                                 lookahead: float = 0.5) -> tuple:
    """
    ...
    for drift_vec in drift_traj:
        ...
        mean_pred = np.clip(opt.mean + lookahead * velocity,
                            BOUNDS[:, 0], BOUNDS[:, 1])
...
def _ppo_gae(rewards: np.ndarray, values: np.ndarray, next_value: float,
...
def main():
    ...
    parser.add_argument("--nu",              type=float, default=1000.0,
                        help="Target T_z/T_x bias")
    parser.add_argument("--lam",             type=float, default=0.5,
                        help="Bias penalty weight")
    parser.add_argument("--n-steps",         type=int,   default=300,
                        help="Online drift steps (Phase B & C)")
    parser.add_argument("--n-cmaes-epochs",  type=int,   default=30,
                        help="Static CMA-ES epochs (Phase A)")
    parser.add_argument("--n-adam-steps",    type=int,   default=100,
                        help="Static Adam steps (Phase A)")
    parser.add_argument("--batch",           type=int,   default=8,
                        help="CMA-ES population size")
    parser.add_argument("--lr",              type=float, default=0.02,
                        help="Adam learning rate")
    parser.add_argument("--drift-seed",      type=int,   default=0,
                        help="RNG seed for drift trajectory")
    parser.add_argument("--drift-period",    type=int,   default=DRIFT_PERIOD,
                        help="Sinusoidal drift period in steps (larger = slower drift)")
    parser.add_argument("--optimizer-freq",  type=int,   default=3,
    ...
    print(f"   Re(g₂):  [{drift_traj[:,0].min():.3f}, {drift_traj[:,0].max():.3f}] MHz")
    print(f"   Re(ε_d): [{drift_traj[:,2].min():.3f}, {drift_traj[:,2].max():.3f}] MHz")
    ...
    print(f"\n── Phase C: Online CMA-ES ({args.n_steps} steps, "
          f"batch={args.batch}) ──")
    Tx_oc, Tz_oc, params_oc = run_online_cmaes(
        batched, loss_fn, x_cmaes, drift_traj, args.batch,
    ...
    Tx_oa, Tz_oa, params_oa = run_online_adam(
        loss_fn, x_adam, drift_traj, args.lr,
        optimizer_freq=args.optimizer_freq, batched_loss=batched)
    ...
    Tx_pc, Tz_pc, params_pc = run_online_cmaes_predictive(
        batched, loss_fn, x_cmaes, drift_traj, args.batch,
        optimizer_freq=args.optimizer_freq, lookahead=args.lookahead)
    ...
    ax = axes2[0]
    ax.plot(st, drift_traj[:, 0], lw=0.9, color="C0",
            label="Total (sinusoidal + OU)")
    ax.plot(st, slow_traj[:, 0], "r--", lw=1.5, alpha=0.8,
            label=f"Sinusoidal (±{DRIFT_AMP_G2} MHz)")
    ax.fill_between(st, drift_traj[:, 0], slow_traj[:, 0],
                    alpha=0.18, color="C0", label="OU noise component")
    ax.axhline(0, color="k", lw=0.5)
    ax.set(xlabel="Drift step", ylabel="Δ Re(g₂) (MHz)", title="g₂ drift")
    ax.legend(fontsize=8); ax.grid(True, alpha=0.3)

    ax = axes2[1]
    ax.plot(st, drift_traj[:, 2], lw=0.9, color="C1",
            label="Total (sinusoidal + OU)")
    ax.plot(st, slow_traj[:, 2], "r--", lw=1.5, alpha=0.8,
            label=f"Sinusoidal (±{DRIFT_AMP_EPS} MHz)")
    ax.fill_between(st, drift_traj[:, 2], slow_traj[:, 2],
                    alpha=0.18, color="C1", label="OU noise component")
    ax.axhline(0, color="k", lw=0.5)
    ...
    ax.set(xlabel="Drift step", ylabel="T_z (µs)",
    ...
    ax.axhline(args.nu, color="r", linestyle=":", lw=1.2,
    ...
    fig4.suptitle(
        "Phase C — Online stabilization  "
        "(continuous re-optimization tracks drift)",
        fontsize=12,
    )
    ...
    ax.semilogy(st, _smth(Tz_sc,  W), label="CMA-ES static", alpha=0.35, color="C0", linestyle=":")
    ax.semilogy(st, _smth(Tz_sa,  W), label="Adam static",   alpha=0.35, color="C1", linestyle=":")
    ax.set(xlabel="Drift step", ylabel="T_z (µs)",
           title="T_z  (online vs static)")
    ax.legend(fontsize=8); ax.grid(True, alpha=0.3)
    ...
    ax.semilogy(st, _smth(Tx_sc,  W), label="CMA-ES static", alpha=0.35, color="C0", linestyle=":")
    ax.semilogy(st, _smth(Tx_sa,  W), label="Adam static",   alpha=0.35, color="C1", linestyle=":")
    ax.set(xlabel="Drift step", ylabel="T_x (µs)",
           title="T_x  (online vs static)")
    ax.legend(fontsize=8); ax.grid(True, alpha=0.3)
    ...
    ax.semilogy(st, _smth(ratio_sc,  W), label="CMA-ES static", alpha=0.35, color="C0", linestyle=":")
    ax.semilogy(st, _smth(ratio_sa,  W), label="Adam static",   alpha=0.35, color="C1", linestyle=":")
    ax.axhline(args.nu, color="r", linestyle=":", lw=1.2,
               label=f"target ν={args.nu:.0f}")
    ax.set(xlabel="Drift step", ylabel="T_z / T_x",
           title="Noise bias  (online vs static)")
    ax.legend(fontsize=8); ax.grid(True, alpha=0.3)
    ...
    ax.plot(st, np.full(args.n_steps, x_cmaes[0]), "k:", lw=0.8,
            label=f"Static optimum ({x_cmaes[0]:.3f})")
    ax.plot(st, x_cmaes[0] + ideal_g2_corr, "k--", lw=0.8, alpha=0.5,
            label="Ideal (static − drift)")
    ax.set(xlabel="Drift step", ylabel="Re(g₂) [MHz]",
           title="Re(g₂) nominal set point")
    ax.legend(fontsize=7); ax.grid(True, alpha=0.3)

    ax = axes4[1, 1]
    ax.plot(st, params_oc[:, 2], label="CMA-ES nominal Re(ε_d)")
    ax.plot(st, params_oa[:, 2], label="Adam nominal Re(ε_d)", linestyle="--")
    ax.plot(st, np.full(args.n_steps, x_cmaes[2]), "k:", lw=0.8,
            label=f"Static optimum ({x_cmaes[2]:.3f})")
    ax.plot(st, x_cmaes[2] + ideal_eps_corr, "k--", lw=0.8, alpha=0.5,
            label="Ideal (static − drift)")
    ax.set(xlabel="Drift step", ylabel="Re(ε_d) [MHz]",
           title="Re(ε_d) nominal set point")
    ax.legend(fontsize=7); ax.grid(True, alpha=0.3)
    ...
    ax.set(xlabel="Drift step", ylabel="Δ Re(g₂) [MHz]",
    ...
    ls = ["-", "--", "-.", ":", (0, (3, 1, 1, 1))]
    labels = ["CMA-ES standard", "Adam standard", "CMA-ES predictive", "Adam predictive", "PPO"]
    x_effs_g2  = [x_eff_oc[:, 0], x_eff_oa[:, 0], x_eff_pc[:, 0], x_eff_pa[:, 0], x_eff_ppo[:, 0]]
    x_effs_eps = [x_eff_oc[:, 2], x_eff_oa[:, 2], x_eff_pc[:, 2], x_eff_pa[:, 2], x_eff_ppo[:, 2]]
    ...
    ax.axhline(x_cmaes[0], color="k", lw=1.2, linestyle="--", label=f"Ideal target ({x_cmaes[0]:.3f})")
    ax.set(xlabel="Drift step", ylabel="Re(g₂) effective [MHz]",
    ...
    ax.axhline(x_cmaes[2], color="k", lw=1.2, linestyle="--", label=f"Ideal target ({x_cmaes[2]:.3f})")
    ax.set(xlabel="Drift step", ylabel="Re(ε_d) effective [MHz]",
    ...
    ax.set(xlabel="Drift step", ylabel="Error in Re(g₂) [MHz]",
    ...
    ax.axhline(0, color="k", lw=1.0)
    ax.set(xlabel="Drift step", ylabel="Error in Re(ε_d) [MHz]",
    ...
    lbls6   = ["CMA-ES std", "Adam std", "CMA-ES pred", "Adam pred", "PPO", "Static (no FB)"]
    ls6     = ["-", "--", "-.", ":", "-.", ":"]
    ...
    ax.axhline(args.nu, color="r", linestyle=":", lw=1.5,
               label=f"target ν={args.nu:.0f}")
    ax.set(xlabel="Drift step", ylabel="T_z / T_x",
